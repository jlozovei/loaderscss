...
// task styles
gulp.task('styles', () => {
	return gulp.src(`${config.cssDevelopmentPath}/*.css`)
		.pipe(plumber())
...
// task scripts
gulp.task('scripts', () => {
	return gulp.src(`${config.jsDevelopmentPath}/*.js`)
		.pipe(jsHint())
		.pipe(webpack({
			module: {
				loaders: [{
...
// task templates -- only to refresh browser on html file changes
gulp.task('templates', () => {
...
gulp.task('default', () =>{
	...
	gulp.watch(`${config.cssDevelopmentPath}/**/*.css`, ['styles'])
	gulp.watch(`${config.jsDevelopmentPath}/**/*.js`, ['scripts'])
